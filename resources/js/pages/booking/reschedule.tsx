...
type Lot = { id: number; lot_number: string; size: string; normalized_size?: string }
...
export default function BookingReschedule() {
  ...
  const [state, setState] = React.useState<BookingState>({
    ...
    grave_type: booking.activity_type !== "ziarah" ? "makam" : booking.grave_type,
    ...
    burn_barrels_count: booking.facilities.burn_barrels_count,
    has_tent: booking.facilities.has_tent,
    has_prayer_table: booking.facilities.has_prayer_table,
    has_lamp: booking.facilities.has_lamp,
  ...
  const didInitLotsRef = React.useRef(false)
  ...
  React.useEffect(() => {
    ...
    if (didInitLotsRef.current) {
      setState((prev) => ({ ...prev, lot_id: null, lot_search: "" }))
    } else {
      didInitLotsRef.current = true
    }
        ...
        <div className="wrap">
          {expired ? (
            <div className="hint-box" style={{ marginBottom: 12 }}>
              <p>Masa berlaku aksi sudah habis.</p>
            </div>
          ) : null}
              ...
              {state.step === 4 ? (
                <div className="space">
                  ...
                  <button
                    ...
                    disabled={expired || postForm.processing}
                  >
